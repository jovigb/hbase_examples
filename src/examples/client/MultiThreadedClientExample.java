...
public class MultiThreadedClientExample extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		configuration.set("hbase.zookeeper.quorum", "192.168.2.180,192.168.2.179,192.168.2.178");
		configuration.set("hbase.master", "192.168.2.179:60000");
		...
		final Connection writeConnection = ConnectionFactory.createConnection(configuration, service);// getConf()
		final Connection readConnection = ConnectionFactory.createConnection(configuration, service);
		...
		for (int i = 0; i < numOperations; i++) {
			...
			// f = internalPool.submit(new WriteExampleCallable(writeConnection,
			// tableName));
			futures.add(f);
		}
	...
	public static class WriteExampleCallable implements Callable<Boolean> {
		...
		public Boolean call() throws Exception {
			...
			try (Table t = connection.getTable(tableName)) {
				...
				for (int i = 0; i < 30; i++) {
					Long ll = ThreadLocalRandom.current().nextLong();
					byte[] rk = Bytes.toBytes(MultiThreadedClientExample.md5(ll.toString()));
					// byte[] rk =
					// Bytes.toBytes(((Long)java.lang.Math.abs(ThreadLocalRandom.current().nextLong())).toString());
					Put p = new Put(rk);
	...
	public static class ReadExampleCallable implements Callable<Boolean> {
		...
		public Boolean call() throws Exception {
			...
			try (Table t = connection.getTable(tableName)) {
				// byte[] rk =
				// Bytes.toBytes(ThreadLocalRandom.current().nextLong());
				Long ll = ThreadLocalRandom.current().nextLong();
				byte[] rk = Bytes.toBytes(MultiThreadedClientExample.md5(ll.toString()).substring(0, 4));
				Scan s = new Scan(rk);
